...
function Receiver_init(){
	...
	dk.create("DKReceiver/Receiver.html", function(){
		...
		server.addEventListener("message", function onmessage(event){
			...
			console.log("event.value = "+event.value);
			Receiver_onmessage(event.value);
		})
		...
		byId("VolumeUp_Button").addEventListener("click", function volume_up(event){
			Receiver_onevent(event)
		});
		byId("VolumeDown_Button").addEventListener("click", function volume_down(event){
			Receiver_onevent(event)
		});
	});
}
...
function Receiver_onmessage(message){
	console.log("Receiver_onmessage("+message+")");
	if(message == "Power"){
